...
pub struct CheckerConfig {
    pub osmosis_grpc_scheme: String,
    pub osmosis_grpc_host: String,
    pub osmosis_grpc_port: u16,
...
impl CheckerConfig {
    pub fn osmosis_grpc_endpoint(&self) -> String {
        format!(
            "{}://{}:{}",
            self.osmosis_grpc_scheme, self.osmosis_grpc_host, self.osmosis_grpc_port
        )
    }
}
...
impl SelfValidation for CheckerConfig {
    fn validate(&self) -> Result<()> {
        if self.osmosis_grpc_scheme != "http" && self.osmosis_grpc_scheme != "https" {
            return Err(anyhow!(
                "osmosis daemon grpc_scheme must be either 'http' or 'https'..."
            ));
        }
...
impl FromEnv for CheckerConfig {
    fn from_env() -> Result<Self> {
        Ok(Self {
            osmosis_grpc_scheme: "http".to_owned(),
            osmosis_grpc_host: "127.0.0.1".to_owned(),
            osmosis_grpc_port: 9090,
...
impl TryFrom<CheckerToml> for CheckerConfig {
    ...
    fn try_from(toml: CheckerToml) -> Result<Self> {
        ...
        if let Some(osmosis_grpc_scheme) = toml.osmosis_grpc_scheme {
            result.osmosis_grpc_scheme = osmosis_grpc_scheme;
        }
        if let Some(osmosis_grpc_host) = toml.osmosis_grpc_host {
            result.osmosis_grpc_host = osmosis_grpc_host;
        }
        if let Some(osmosis_grpc_port) = toml.osmosis_grpc_port {
            result.osmosis_grpc_port = osmosis_grpc_port;
        }
