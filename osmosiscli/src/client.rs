...
use anyhow::Context;
use logger::prelude::*;
use osmosis_grpc_client as proto;
use osmosis_grpc_client::tonic::{self, transport::Channel, Code};

#[derive(Debug, Clone)]
pub struct OsmosisdClient {
    endpoint: String,
...
impl OsmosisdClient {
    pub async fn establish_connection(endpoint: &str) -> Result<Channel> {
        tonic::transport::Endpoint::new(endpoint.to_owned())?
            ...
            .with_context(|| {
                format!(
                    "failed to connect osmosis daemon grpc endpoint: {}",
                    endpoint
                )
            })
    }
