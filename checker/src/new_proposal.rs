...
use logger::prelude::*;
use osmosis_grpc_client::{self as proto, prost};
...
impl NewProposalChecker {
    pub fn new(osmosisd_endpoint: String, receiver: Receiver<NewProposalMessage>) -> Self {
        Self {
            osmosisd_endpoint,
            receiver,
        }
                                                ...
                                                body.messages.iter().enumerate()
                                            {
                                                    ...
                                                    == "/cosmos.gov.v1beta1.MsgSubmitProposal"
                                                {
                                                    ...
                                                    let message_logs = if tx_message_logs.is_some()
                                                    {
                                                        ...
                                                        let tx_response = osmosiscli::get_client(
                                                            &self.osmosisd_endpoint,
                                                        )
                                                        .lock()
                                                        .await
