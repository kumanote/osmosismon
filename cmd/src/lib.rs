...
pub fn start() -> Result<()> {
    ...
    for checker in &app_config.checkers {
        ...
        let runtime = manager
            .osmosisd_endpoint(checker.osmosis_grpc_endpoint().as_str())
...
impl CheckManager {
    pub fn osmosisd_endpoint(&mut self, osmosisd_endpoint: &str) -> &mut Self {
        self.osmosisd_endpoint = osmosisd_endpoint.to_owned();
        self
    }
    ...
    fn setup(&mut self) -> tokio::runtime::Runtime {
        let runtime = tokio::runtime::Builder::new_multi_thread()
            .thread_name(format!("{}", self.osmosisd_endpoint.as_str()))
            .enable_all()
            .build()
            .expect("Failed to create tokio runtime!");
    ...
    async fn next(&mut self) -> Result<&mut Self> {
        ...
        let latest_block_response = osmosiscli::get_client(&self.osmosisd_endpoint)
            .lock()
            .await
            .fetch_latest_block()
            .await?;
        ...
        if from_height <= latest_height {
            for height in from_height..=latest_height {
                let block_message: BlockMessage = if height == latest_height {
                    ...
                    let block_response = osmosiscli::get_client(&self.osmosisd_endpoint)
                        .lock()
                        .await
