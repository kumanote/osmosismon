...
#[structopt(about = "osmosismon running options")]
struct Opts {
...
fn main() {
    ...
    debug!(
        "Loaded osmosis daemon monitoring tool config, config: {:?}",
        config
    );

    // set up osmosis client pool
    let mut client_pool = HashMap::new();
    for checker in &config.checkers {
        let endpoint = checker.osmosis_grpc_endpoint();
        info!("this tool will connect to {}", endpoint.as_str());
        let client = OsmosisdClient::new(endpoint.clone());
        client_pool.insert(endpoint, Arc::new(Mutex::new(client)));
    }
    osmosiscli::set_client_pool(client_pool);

    if let Err(err) = osmosismon::start() {
        error!("{}", err);
    }
